  ...
  if(bulkInsertData.length > 0) {
    await Promise.all(bulkInsertData.map( async (rev, index) => {
      ...
      const createduser = await Users.create([{
        ...
        nationality:itemObject.nationality,
        ...
        isAdministrator: itemObject.isAdministrator,
        position:itemObject.position,
        isChitCommitteeMember:itemObject.isChitCommitteeMember,
        chitCommitteePosition:itemObject.chitCommitteePosition
      }], { session });
      const createduserId = JSON.parse(JSON.stringify(createduser));
...
const getuserDetail = async ({_id,session}) => {
  try {
    ...
    let familyMembers = [];
    const familyData = await FamilyDetails.findOne({ memberId: _id }).exec();
    if (familyData) {
        familyMembers = await FamilyDetails.find({ familyId: familyData.familyId }).exec();
    }
...
const userupdate = async ({ reqBody, session }) => {
  ...
  const _id = parsedData.member_id;
  const existingvalueData = await Users.findOne({_id});
  existingvalueJson.username = existingvalueData.username;
  existingvalueJson.password = existingvalueData.password;
  existingvalueJson.roles = existingvalueData.roles;
...
const familymemberscreate = async ({ reqBody, session }) => {
}
  ...
  if(parsedData.length > 0) {
    try {
      for (const record of parsedData) {
        if(record._id){
          const existingRecord = await familyDetails.findOne({ _id: record._id });
          await familyDetails.updateOne({ _id: existingRecord._id }, record);
        } else {
          delete record._id;
          let familyIdcount = "";
          let familySubId = "";
          ...
          if(record.isnewFamily) {
            ...
            if(record.memberId) {
              ...
              await Users.findOneAndUpdate({_id:record.memberId}, newvalues)
            }
          } else {
...
module.exports = {
  createuser, createusercount, userslist, edituser, deleteuser, loghistory, loghistorylist, bulkInsert, getuserDetail,userSearch,userupdate,familymemberscreate,familymembersupdate
};
