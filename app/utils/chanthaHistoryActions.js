...
const chanthaHistoryCronRun = async () => {
  ...
  const userData = await User.find(combinedQuery);
  ...
  var chanthaDatas = await Chantha.find(chanthaQuery);
  var chanthaData = chanthaDatas[0];
  await Promise.all(userData.map(async (user) => {
    await createChanthaHistory(user, chanthaData);
  }));
  console.log("Chantha History added successfully!")
}

async function createChanthaHistory(user, chanthaData) {
  ...
  const chanthaHistory = new ChanthaHistory(chanthaHistoryData);
  await chanthaHistory.save();
  const tempAmount = user.balanceTribute;
  user.balanceTribute = parseInt(tempAmount) + parseInt(chanthaData.amount);
  await user.save();
}

module.exports = {
  createchanthaHistory, chanthaHistoryList, editchanthaHistory, deletechanthaHistory, loghistory, chanthaHistoryCronRun
};
