...
// const familyLevelDetails = async ({ reqBody, session }) => {
//   let familyLevels = [];
//   const family_Details = await familyDetails.find({ 'familyId': reqBody.familyId });

//   if (family_Details) {
//     const uniqueSubIds = new Set();
//     family_Details.forEach(detail => {
//       uniqueSubIds.add(detail.subId);
//     });
//     familyLevels = familyLevels.concat(Array.from(uniqueSubIds));
//   }
//   console.log(familyLevels);
//   return {
//     status: true,
//     statusCode: 201,
//     data: familyLevels
...
const familyLevelDetails = async ({ reqBody, session }) => {
  ...
  if (family_Details) {
    ...
    // Transform uniqueSubIds into the required DropdownItem format
    familyLevels = Array.from(uniqueSubIds).map(subId => {
      const detail = family_Details.find(detail => detail.subId === subId);
      return {
        value: subId.toString(),
        label: `${detail ? detail.firstname + ' ' + detail.lastname : ''}`,
      };
    });
  }
...
module.exports = {
  createfamilyDetails, familyDetailsList, editfamilyDetails, deletefamilyDetails, loghistory, familyLevelDetails
};
